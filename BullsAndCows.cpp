        ...
        for(int i=0; i<secret.size(); i++){
            ...
            if (s == g){
              bulls++; 
                ...
                if (num[s] < 0) cows++;   // means now we found a number in serect that match the number found in guess before.
                if (num[g] > 0) cows++;    // a number in guess match the number in secret && this number didn't be matched.
                num[s]++;
                num[g]--;
            }
        }
...
public:
    ...
    string getHint(string secret, string guess) {
        ...
        for (int i = 0; i < sVec.size(); ++i) {
            bCnt += min(sVec[i], gVec[i]);          // trick is in this line
        }
